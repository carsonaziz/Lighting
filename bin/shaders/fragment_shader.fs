...
uniform sampler2D meshMapTexture;
uniform int hasMap;
...
void main() {
	...
	if(hasMap == 1) {
		if(texture(meshMapTexture, outTextCoord) == vec4(1.0, 0.0, 0.0, 1.0)) {
			finalColour = texture(meshTexture, outTextCoord);
		} else {
			finalColour = vec4(result, 1.0) * texture(meshTexture, outTextCoord);
		}
	} else {
		finalColour = vec4(result, 1.0) * texture(meshTexture, outTextCoord);
	}
