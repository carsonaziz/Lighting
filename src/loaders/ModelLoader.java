...
public class ModelLoader {
	...
	public static Model loadModel(String modelPath, String texturesDir) throws Exception {
		return loadModel(modelPath, texturesDir, aiProcess_GenSmoothNormals 
	...
	private static Model loadModel(String modelPath, String texturesDir, int flags) throws Exception {
		...
		for(int i = 0; i < numMeshes; i++) {
			...
			meshes[i] = mesh;
			mesh.setMaterial(createMaterial(texturesDir));
		}
		
		return new Model(meshes, rootTransformation);
	}
	
	public static AnimatedModel loadAnimatedModel(String modelPath, String animationsDir, String texturesDir) throws Exception {
		return loadAnimatedModel(modelPath, animationsDir, texturesDir, aiProcess_GenSmoothNormals 
				| aiProcess_JoinIdenticalVertices 
				| aiProcess_Triangulate 
				| aiProcess_FixInfacingNormals 
				| aiProcess_LimitBoneWeights);
	...
	private static AnimatedModel loadAnimatedModel(String modelPath, String animationsDir, String texturesDir, int flags) throws Exception {
		AIScene aiScene = aiImportFile(modelPath, flags);
		...
		return new AnimatedModel(meshes, animations, rootNode, rootTransformation);
	}
	...
	private static Material createMaterial(String texturesDir) {
		...
		if(textures.size() == 1) {
			material = new Material(textures.get(0), null);
		} else if(textures.size() == 2) {
			material = new Material(textures.get(0), textures.get(1));
			material.setHasMap(true);
		}
